"use client";

import {BsArrowDownRight} from "react-icons/bs"
import Link  from "next/link";


const services = [
    {
    num:'01',
    title:'Web Development',
    description:'We deliver custom, responsive, and performance-driven websites designed to elevate your brand.',
    href:''
},
{
    num:'02',
    title:'UI/UX Design',
    description:'We deliver custom, responsive, and performance-driven websites designed to elevate your brand.',
    href:''
},
{
    num:'03',
    title:'Logo Design',
    description:'We deliver custom, responsive, and performance-driven websites designed to elevate your brand.',
    href:''
},
{
    num:'04',
    title:'Web Pentesting',
    description:'We deliver custom, responsive, and performance-driven websites designed to elevate your brand.',
    href:''
},
]

import { motion } from "framer-motion";

const Services =()=>{
    return (
    <section className="min-h-[80vh] flex flex-col justify-center py-12 xl:py-0">
        <div className="container mx-auto">
         <motion.div
          initial={{ opacity:0 }}
          animate={{ 
            opacity:1,
            transition:{delay: 2.4 ,duration:0.4 ,ease:"easeIn"}
           }}
           className="grid grid-cols-1 md:grid-cols-2 gap-[60px]"
         >
                {services.map((service,index)=>{
                    return <div key={index} 
                    className="flex-1 flex flex-col justify-center gap-6 group">
                        {/* TOP */}
                        <div className="w-full flex justify-between items-center">
                            {/* Number style */}
                            <div className="text-5xl font-extrabold text-outline text-transparent group-hover:text-outline-hover">
                                {service.num}
                            </div>
                            {/*Links style */}
                            <Link href={service.href} className="w-[70px] h-[70px] rounded-full bg-white group-hover:bg-cyan-500 transition-all duration-500 flex justify-center items-center hover:-rotate-45">
                            <BsArrowDownRight className="text-stone-950  text-3xl"/>
                            </Link>
                        </div>
                            {/* Title style */}
                        <h2 className="text-[42px] font-bold leading-none text-white group-hover:text-cyan-500 transition-all duration-500 ">
                            {service.title}
                            </h2>
                            {/* Description style */}
                        <p className="text-white/60">
                            {service.description}
                        </p>
                            {/* Border style */}
                        <div className="border-b border-white/20 w-full"></div>
                    </div>
                })}
         </motion.div>
        </div>
 
    </section>
    )
}

export default Services